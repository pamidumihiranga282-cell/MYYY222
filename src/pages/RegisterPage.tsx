...
const RegisterPage: React.FC<RegisterPageProps> = ({ setCurrentPage }) => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      toast.success('Welcome to MRM Shopping! 🎉');
      setCurrentPage('home');
    } catch (err: any) {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-gradient-to-r from-gold-500 to-gold-400 p-8 text-center">
          <div className="w-20 h-20 rounded-full bg-white/20 flex items-center justify-center mx-auto mb-4 backdrop-blur-sm">
            <UserPlus size={36} className="text-chocolate-900" />
          </div>
          <h1 className="font-display text-2xl font-bold text-chocolate-900">Create Account</h1>
          <p className="text-chocolate-700 mt-1">Join MRM Shopping 🛍</p>
        </div>
        ...
        <div className="p-8">
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-chocolate-400">
            ...
            <div>
              <label className="text-sm font-medium text-chocolate-700 mb-1 block">Confirm Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-chocolate-400" size={18} />
                <input
                  type={showPassword ? 'text' : 'password'}
          ...
          <p className="text-center text-sm text-chocolate-500 mt-6">
            ...
            <button onClick={() => setCurrentPage('login')} className="text-gold-600 font-semibold hover:underline">
              Sign in here
            </button>
          </p>
        </div>
